...
function showSilhouetteQuiz(area) {
  ...
  // エリアに応じた画像を表示
  if (area === "desk") {
    silhouetteImg.src = "images/noa_puzzle.png";  // 例：デスク領域の場合（回答「ノア」）
  } else {
    silhouetteImg.src = "images/roberia_puzzle.png"; // 例：ドライブ領域の場合（回答「ロベリア」）
  }
  ...
  quizModal.appendChild(answerBtn);

  answerBtn.addEventListener("click", () => {
    // 入力欄がまだない場合のみ作成
    let inputField = quizModal.querySelector("input[type='text']");
    if (!inputField) {
      ...
      quizModal.appendChild(submitBtn);

      submitBtn.addEventListener("click", () => {
        const answer = inputField.value.trim();
        // エリアに応じた正解チェック（デスク→「ノア」、ドライブ→「ロベリア」）
        if (area === "desk") {
          if (answer === "ノア") {
            alert("正解！");
