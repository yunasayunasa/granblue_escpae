...
// ==============================
// エリア3ゲーム開始処理（選択肢に応じたゲーム遷移）
// ==============================
function startArea3Game(choice) {
  // choice: "A" ならクイズ、"B" なら謎解き
  if (choice === "A") {
    startQuizGame();
  } else if (choice === "B") {
    startRiddleGame();
...
function startQuizGame() {
  ...
  function showQuizQuestion() {
    ...
    questionElem.style.fontSize = "1.5rem";
    questionElem.style.textAlign = "center";
    questionElem.textContent = q.question;
    quizModal.appendChild(questionElem);
    ...
    quizModal.appendChild(optionsContainer);
...
// ==============================
// 謎解きゲーム実装（ノアネガ戦：フリーワード3問）
// ==============================
function startRiddleGame() {
  const riddleModal = document.createElement("div");
  riddleModal.id = "riddle-modal";
  riddleModal.style.position = "absolute";
  riddleModal.style.top = "0";
  riddleModal.style.left = "0";
  riddleModal.style.width = "100%";
  riddleModal.style.height = "100%";
  riddleModal.style.backgroundColor = "rgba(0,0,0,0.8)";
  riddleModal.style.zIndex = "4000";
  riddleModal.style.display = "flex";
  riddleModal.style.flexDirection = "column";
  riddleModal.style.justifyContent = "center";
  riddleModal.style.alignItems = "center";
  document.body.appendChild(riddleModal);

  const riddleQuestions = [
    {
      type: "anagram",
      question: "アナグラム： 以下の文字を正しく並べ替えて答えよ： またごきや",
      answer: "たまごやき"
    },
    {
      type: "aha",
      question: "次の画像をよく見よ。（画像がゆっくり変化するアニメーションを実装済み）今の画像から増えた（消えた）物は？",
      answer: "ゴリラ"
    },
    {
      type: "number",
      question: "数字パズル： 1＋4＝5, 2＋5＝12, 3＋6＝21, 8＋11＝□　さて□に入る数字は？",
      answer: "96"
    }
  ];
  ...
  if (currentRiddleIndex >= riddleQuestions.length) {
    alert("謎解き全問正解！ エリア3クリア！");
    document.body.removeChild(riddleModal);
    showEndingLines();
    return;
  }
  ...
  // 共通：質問文（タイプにより表示タイミングが変わる）
  // ※通常タイプはすぐ表示、"aha" タイプは後で表示するため、ここでは未追加
  
  if (q.type === "aha") {
    // アハ体験型：画像のゆっくりクロスフェードを実装
    ...
    initialImg.src = "images/" + q.initialImage;
    ...
    initialImg.style.transition = "opacity 15s linear";
    
    const finalImg = document.createElement("img");
    finalImg.src = "images/" + q.finalImage;
    ...
    finalImg.style.transition = "opacity 15s linear";
    
    imageContainer.appendChild(initialImg);
    imageContainer.appendChild(finalImg);
    riddleModal.appendChild(imageContainer);
    ...
    // 15秒後（1秒+15秒=16秒後）に画像コンテナを削除して、問題文と入力フォームを表示
    setTimeout(() => {
      imageContainer.remove();
      displayRiddleInput(q);
    }, 16000);
  } else {
    // 通常タイプ：すぐに問題文と入力フォームを表示
    displayRiddleInput(q);
  }
}

// displayRiddleInput: 質問文、入力欄、送信ボタンを表示する共通処理
function displayRiddleInput(q) {
  // 質問文
  const questionElem = document.createElement("p");
  questionElem.style.color = "#fff";
  questionElem.style.fontSize = "1.5rem";
  questionElem.style.textAlign = "center";
  questionElem.textContent = q.question;
  riddleModal.appendChild(questionElem);
  ...
  riddleModal.appendChild(inputField);
  ...
  riddleModal.appendChild(submitBtn);
...
// ==============================
// エンディングナレーション表示（エリア3エンディングへ遷移）
// ==============================
function showEndingLines() {
  // 簡単なエンディングナレーションの例として、エンディング画像とテキストを表示
  const scene = document.createElement("div");
  scene.id = "narration-screen3";
  scene.className = "scene";
  scene.style.position = "absolute";
  ...
  // エンディングナレーションのシナリオデータ（endingLines）を順に表示
  let idx = 0;
  function endClickHandler() {
    if (idx >= endingLines.length) {
      scene.removeEventListener("click", endClickHandler);
      scene.removeEventListener("touchstart", endClickHandler);
      showEndingImage(scene);
      return;
    }
    const line = endingLines[idx++];
    updateCharactersInEnding(scene, line.characters);
    textContainer.innerHTML = `<p>${line.text}</p>`;
  }
  scene.addEventListener("click", endClickHandler);
  scene.addEventListener("touchstart", endClickHandler);
...
function showEndingImage(scene) {
  // シーン内容をクリアしてエンディング画像とテキストを表示
  scene.innerHTML = "";
  scene.style.backgroundImage = "url('images/title.jpg')";
  scene.style.backgroundSize = "contain";
  scene.style.backgroundPosition = "center";
